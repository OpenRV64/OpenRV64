...
`v2`, loads the two corresponding B vectors into `v3`/`v7`, launches products
into `v5`/`v4`, and accumulates independently into `v8`/`v9`. The crossed
product/accumulator parity avoids same-bank operand pairs. Explicit `VSYNC`s
protect only actual load, product, accumulator, and store dependencies. An odd
vector-column count falls through a one-block tail.
...
`A[M][K][16]` and `B[N/16][K][16]`. Multiply and add remain distinct vector
instructions and therefore round to BF16 between the two operations. The
