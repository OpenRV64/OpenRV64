...
$(FD_UOP_HARNESS_COMPACT_MUL_SIM_BUILD): $(FD_UOP_HARNESS_SIM_SRCS) \
		$(FD_DISPATCH_SRCS) $(FPR_SRCS) $(FPU_SRCS) $(FP_ISA_SRCS)
	mkdir -p sim
	iverilog -g2012 -Wall -Irtl -s tb_fd_uop_harness \
		-Ptb_fd_uop_harness.ENABLE_PIPELINED_MULTIPLY=0 \
		-o $(FD_UOP_HARNESS_COMPACT_MUL_SIM_BUILD) \
		$(FD_DISPATCH_SRCS) $(FPR_SRCS) $(FPU_SRCS) \
		$(FD_UOP_HARNESS_SIM_SRCS)
...
$(RECURSIVE_LOCK_CONTEXT_SIM_BUILD): \
		$(RECURSIVE_LOCK_CONTEXT_SIM_SRCS) $(CORE_SRCS) $(ISA_SRCS) \
		$(ARITH_DEPS) $(BP_DEPS)
	mkdir -p sim
	iverilog -g2012 -Wall -Irtl -o \
		$(RECURSIVE_LOCK_CONTEXT_SIM_BUILD) \
		$(RECURSIVE_LOCK_CONTEXT_SIM_SRCS) $(CORE_SRCS)

$(WFI_CONTEXT_SIM_BUILD): $(WFI_CONTEXT_SIM_SRCS) $(CORE_SRCS) \
		$(ISA_SRCS) $(ARITH_DEPS) $(BP_DEPS)
	mkdir -p sim
	iverilog -g2012 -Wall -Irtl -o $(WFI_CONTEXT_SIM_BUILD) \
		$(WFI_CONTEXT_SIM_SRCS) $(CORE_SRCS)
...
$(EXEC_BP_TAGE_SIM_BUILD): tb/tb_exec_bp_tage.sv $(BP_SRC) $(BP_DEPS)
	mkdir -p sim
	iverilog -g2012 -Wall -Irtl -o $(EXEC_BP_TAGE_SIM_BUILD) \
		rtl/core/exec/bp/bp.v tb/tb_exec_bp_tage.sv
...
$(EXEC_FPU_RV64FD_COMPACT_MUL_SIM_BUILD): $(EXEC_FPU_RV64FD_SIM_SRCS) \
		$(FPU_SRCS) $(FP_ISA_SRCS)
	mkdir -p sim
	iverilog -g2012 -Wall -Irtl \
		-Ptb_exec_fpu_rv64fd.ENABLE_PIPELINED_MULTIPLY=0 \
		-o $(EXEC_FPU_RV64FD_COMPACT_MUL_SIM_BUILD) \
		$(EXEC_FPU_RV64FD_SIM_SRCS)
