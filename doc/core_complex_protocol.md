...
The first implementation is `openrv64_icx_protocol_wrapper_1h` in
...
ICX request and response channels:

```text
OpenRV64 physical port
        |
        v
hart_legacy_adapter  -- ICX request/response --  axi_master
                                                   |
                                                   v
                                             external AXI4
```
...
The next generated implementation is `openrv64_icx_protocol_wrapper_nh`.
`openrv64_icx_protocol_wrapper_2h` and
`openrv64_icx_protocol_wrapper_4h` fix that implementation at two and four
...
The currently implemented scalar compatibility ICX request carries:
...
This channel is a bring-up seam and is not the native cache-facing ICX
datapath.

## Native 512-bit cache-line request

The required native interface above ICX is 512 bits wide. One accepted data
beat carries exactly one 64-byte cache line. Cacheable refills, evictions,
interventions, and prefetches therefore cross ICX one complete line at a time
...
boundaries. ICX itself does not inherit AXI's 4 KiB burst restriction; the
...
channels may handshake independently, but the ICX write completes only after
...
AXI error responses become ICX errors and then ordinary core bus errors.

Only ICX READ and WRITE are translated today. Reserved operations complete
...
the native 512-bit ICX channels directly, inserts a shared L2, and presents an
