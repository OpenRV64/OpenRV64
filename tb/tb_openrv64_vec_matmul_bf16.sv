...
module tb_openrv64_vec_matmul_bf16;

    localparam logic [63:0] RESET_VECTOR = 64'h100;
    localparam integer RESET_WORD = RESET_VECTOR >> 3;
    localparam integer MEM_WORDS = 512;
    localparam integer C_BASE_WORD = 32'h0a00 >> 3;
    ...
    wire ifetch_in_range = ifetch_addr[63:12] == 0;
    assign ifetch_ready = ifetch_valid;
    assign ifetch_rdata = ifetch_in_range ?
        memory[ifetch_addr[11:3]] : 64'd0;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (vec_mem_req_valid && vec_mem_req_ready) begin
                ...
                response_data_q <= (vec_mem_req_addr[63:12] == 0) ?
                    memory[vec_mem_req_addr[11:3]] : 64'd0;
                response_error_q <= vec_mem_req_addr[63:12] != 0;
                response_retry_q <= !retry_used_q &&
                                    (vec_mem_req_addr == 64'h808);
                if (!retry_used_q && (vec_mem_req_addr == 64'h808))
                    retry_used_q <= 1'b1;

                if (vec_mem_req_write) begin
                    if (vec_mem_req_wstrb != 8'hff)
                        $fatal(1, "BF16 matmul store was not a full beat");
                    if (vec_mem_req_addr[63:12] != 0)
                        $fatal(1, "BF16 matmul store escaped memory");
                    memory[vec_mem_req_addr[11:3]] <= vec_mem_req_wdata;
                end
            end
        end
    end

    function automatic [15:0] exact_uint_to_bf16;
        input integer value;
        integer exponent;
        integer significand;
        ...
        begin
            if (value == 0) begin
                exact_uint_to_bf16 = 16'd0;
            end else begin
                exponent = 0;
                while ((1 << (exponent + 1)) <= value)
                    exponent = exponent + 1;
                if (exponent <= 7)
                    significand = value << (7 - exponent);
                else
                    significand = value >> (exponent - 7);
                biased_exponent = exponent + 127;
                exact_uint_to_bf16 =
                    {1'b0, biased_exponent, significand[6:0]};
            end
        end
    endfunction
    ...
    always @(posedge clk) begin
        if (rst_n && $test$plusargs("trace_vec_matmul_bf16")) begin
            if (ifetch_valid && ifetch_ready)
    ...
    initial begin
        ...
        if (!$value$plusargs("memh=%s", memh_path))
            $fatal(1, "use +memh=<matmul_bf16.memh>");
        ...
        timeout = 0;
        while (!dbg_halted && timeout < 100000) begin
            @(posedge clk);
            timeout = timeout + 1;
        end
        @(negedge clk);
        ...
        if (!dbg_halted || dbg_error)
            $fatal(1,
                "BF16 matmul failed halt=%0b error=%0b pc=%x instr=%x",
            ...
            for (column = 0; column < 32; column = column + 1) begin
                ...
                if (actual_bf16 !== expected_bf16)
                    $fatal(1,
                        ...
                        row, column, actual_bf16, expected_bf16);
            end
        end
