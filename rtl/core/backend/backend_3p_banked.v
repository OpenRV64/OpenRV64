...
// Conservative banked-register-file fork of the three-pipe backend. Decode
// remains three-wide, while dispatch and retirement are initially limited to
// two instructions so the physical file needs at most four reads and two
// writes. Keep this separate from backend_3p while the handshake path settles.
module openrv64_backend_3p_banked #(
    ...
    parameter [2:0] BRANCH_COMPLETION_FORWARD_MASK = 3'b000,
    ...
    wire [4*PHYS_REG_ADDR_WIDTH-1:0] gpr_read_addr;
    wire [4*`RV64_XLEN-1:0] gpr_read_data;
    wire [3:0] gpr_read_req;
    wire [3:0] gpr_read_valid;
    wire [1:0] gpr_write;
    wire [1:0] gpr_write_valid;
    wire [2*PHYS_REG_ADDR_WIDTH-1:0] gpr_write_addr;
    wire [2*`RV64_XLEN-1:0] gpr_write_data;
    ...
    // complete two-entry group. This breaks the alloc-valid/ready loop and
    // leaves exact-width admission as a later timing optimization.
    // Occupancy <= DEPTH-2 proves room for the largest possible group, so
    ...
    assign allocation_ready = (retire_occupancy_o <= RETIRE_DEPTH - 2);

    wire [4*`RV64_REG_ADDR_WIDTH-1:0] dispatch_gpr_read_addr;
    wire [3*`OPENRV64_DISPATCH_META_WIDTH-1:0]
        dispatch_allocation_meta;
    wire [2*`RV64_REG_ADDR_WIDTH-1:0] rename_destination_arch;
    wire [1:0] rename_destination_valid;
    wire [2*PHYS_REG_ADDR_WIDTH-1:0] rename_destination_new_phys;
    wire [2*PHYS_REG_ADDR_WIDTH-1:0] rename_destination_old_phys;
    ...
    openrv64_dispatch_3p_banked #(
        ...
        .gpr_read_addr_o(dispatch_gpr_read_addr),
        ...
        .allocation_meta_o(dispatch_allocation_meta),
        ...
        .retire_uses_rs1_i(release_uses_rs1),
        .retire_uses_rs2_i(release_uses_rs2),
        .retire_rs1_addr_i(release_rs1_addr),
        .retire_rs2_addr_i(release_rs2_addr),
        .retire_reg_write_i(release_reg_write),
        .retire_rd_addr_i(release_rd_addr),
    ...
    assign rename_destination_arch[
        ...
        dispatch_allocation_meta[
            0*`OPENRV64_DISPATCH_META_WIDTH + 35 +:
            `RV64_REG_ADDR_WIDTH];
    assign rename_destination_arch[
        1*`RV64_REG_ADDR_WIDTH +: `RV64_REG_ADDR_WIDTH] =
        dispatch_allocation_meta[
            1*`OPENRV64_DISPATCH_META_WIDTH + 35 +:
            `RV64_REG_ADDR_WIDTH];
    assign rename_destination_valid[0] = allocation_valid[0] &&
        dispatch_allocation_meta[
            0*`OPENRV64_DISPATCH_META_WIDTH + 17] &&
        (rename_destination_arch[
            0*`RV64_REG_ADDR_WIDTH +: `RV64_REG_ADDR_WIDTH] !=
         `RV64_REG_X0);
    assign rename_destination_valid[1] = allocation_valid[1] &&
        dispatch_allocation_meta[
            1*`OPENRV64_DISPATCH_META_WIDTH + 17] &&
        (rename_destination_arch[
            1*`RV64_REG_ADDR_WIDTH +: `RV64_REG_ADDR_WIDTH] !=
         `RV64_REG_X0);

    openrv64_rename_identity_banked #(
        .ARCH_ADDR_WIDTH(`RV64_REG_ADDR_WIDTH),
        .ARCH_REG_COUNT(32),
        .PHYS_ADDR_WIDTH(PHYS_REG_ADDR_WIDTH),
        .PHYS_REG_COUNT(PHYS_REG_COUNT)
    ) u_rename (
        .source_arch_i(dispatch_gpr_read_addr),
    ...
    genvar rename_lane;
    generate
        for (rename_lane = 0; rename_lane < 2;
             rename_lane = rename_lane + 1) begin : g_rename_meta
            assign allocation_meta[
                rename_lane*RETIRE_META_WIDTH +: RETIRE_META_WIDTH] = {
                rename_destination_old_phys[
                    rename_lane*PHYS_REG_ADDR_WIDTH +:
                    PHYS_REG_ADDR_WIDTH],
    ...
    endgenerate
    assign allocation_meta[
        2*RETIRE_META_WIDTH +: RETIRE_META_WIDTH] =
        {RETIRE_META_WIDTH{1'b0}};
    ...
    cmn_reg_file #(
        ...
        .wp_addr_i(gpr_write_addr),
        .wp_data_i(gpr_write_data),
        .wp_req_i(gpr_write),
        .wp_valid_o(gpr_write_valid)
    );

    // The queue supplies the ordered head directly. Banked reads wait until
    // retirement has completed any producer write, so no same-edge retirement
    // bypass or execution forwarding is required here.
    ...
    openrv64_exec_top #(
        ...
        .branch_forward_rd_addr_3p_i(`RV64_REG_X0),
...
`ifndef SYNTHESIS
    initial begin
        if (ENABLE_ISSUE_WINDOW != 0)
            $fatal(1, "banked 3p backend does not support the issue window");
        if (ENABLE_SPECULATION_WINDOW != 0)
            $fatal(1,
                   "banked 3p backend does not support speculative issue");
        if ((RELAX_WAW != 0) || (RELAX_HAZARDS != 0))
            $fatal(1, "banked 3p backend requires conservative hazards");
        if ((COMPLETION_FORWARD_MASK != 3'b000) ||
            (BRANCH_COMPLETION_FORWARD_MASK != 3'b000) ||
            (ENABLE_FULL_FORWARDING != 0))
            $fatal(1, "banked 3p backend does not support forwarding yet");
        if ((PHYS_REG_COUNT != 31) || (PHYS_REG_ADDR_WIDTH != 5))
            $fatal(1,
                   "initial banked 3p backend requires 31 physical GPRs");
    end
