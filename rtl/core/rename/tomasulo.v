...
// Dynamic architectural-to-physical rename layer: RAT + free list.
//
// Seeded as a copy of the identity layer; the identity mapping below is a
// placeholder to be replaced by the speculative RAT, per-bank free lists,
// and checkpoint/restore state.  The interface matches identity.v so the
// backend can select either implementation during bring-up.
module openrv64_rename_tomasulo #(
    ...
    initial begin
        ...
        if (PHYS_REG_COUNT < (ARCH_REG_COUNT - 1)) begin
            $fatal(1,
                "tomasulo rename has fewer writable physical registers than architectural registers");
        end
