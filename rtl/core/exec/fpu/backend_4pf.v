...
// Windowed integer plus F/D backend.  Decode supplies up to three static packets per
...
module openrv64_backend_4pf #(
    ...
    input  wire [2:0]                   fp_frm_i,
    output wire                         fp_retire_state_dirty_o,
    output wire                         fp_retire_fflags_valid_o,
    output wire [4:0]                   fp_retire_fflags_o,
    ...
    wire [2:0] allocation_extension_valid;
    wire [6*`RV64_REG_ADDR_WIDTH-1:0] window_gpr_read_addr;
    wire [3*`OPENRV64_DISPATCH_META_WIDTH-1:0]
        window_allocation_meta;
    wire [3*`OPENRV64_FPU_DECODE_PAYLOAD_WIDTH-1:0]
        allocation_extension_payload;
    wire [3*`OPENRV64_EXTENSION_BRANCH_COUNT-1:0]
        allocation_branch_mask;
    wire [ISSUE_WINDOW_DEPTH-1:0] fp_entry_valid;
    wire [ISSUE_WINDOW_DEPTH-1:0] fp_entry_compute;
    wire [ISSUE_WINDOW_DEPTH-1:0] fp_entry_load;
    wire [ISSUE_WINDOW_DEPTH-1:0] fp_entry_store;
    wire [ISSUE_WINDOW_DEPTH-1:0] fp_operand_ready;
    wire [ISSUE_WINDOW_DEPTH*`RV64_XLEN-1:0] fp_entry_store_data;
    ...
    wire fp_mem_issue_is_load;
    wire [`OPENRV64_INSTR_ID_WIDTH-1:0] fp_mem_issue_id;
    wire [SLOT_WIDTH-1:0] fp_mem_issue_slot;
    ...
    wire fpu_ready;
    wire fpu_fire;
    wire [`OPENRV64_INSTR_ID_WIDTH-1:0] fpu_id;
    wire [SLOT_WIDTH-1:0] fpu_slot;
    wire [`OPENRV64_FP_OP_WIDTH-1:0] fpu_op;
    wire [1:0] fpu_fmt;
    wire [2:0] fpu_rm;
    wire [4:0] fpu_type;
    wire [`RV64_XLEN-1:0] fpu_src1;
    wire [`RV64_XLEN-1:0] fpu_src2;
    wire [`RV64_XLEN-1:0] fpu_src3;
    wire [`RV64_REG_ADDR_WIDTH-1:0] fpu_rd;
    wire fpu_fp_reg_write;
    wire fpu_int_reg_write;
    wire fpu_fflags_write;
    wire [`OPENRV64_EXTENSION_BRANCH_COUNT-1:0] fpu_branch_mask;

    localparam integer FPU_TAG_WIDTH =
        `OPENRV64_INSTR_ID_WIDTH + SLOT_WIDTH;
    wire [FPU_TAG_WIDTH-1:0] fpu_request_tag = {fpu_slot, fpu_id};
    wire fpu_result_valid;
    wire fpu_result_ready;
    wire [FPU_TAG_WIDTH-1:0] fpu_result_tag;
    wire fpu_result_is_int;
    wire [`RV64_XLEN-1:0] fpu_result_fp;
    wire [`RV64_XLEN-1:0] fpu_result_int;
    wire [4:0] fpu_result_fflags;
    wire fpu_result_unsupported;

    wire extension_completion_valid;
    wire extension_completion_accept;
    wire [`OPENRV64_INSTR_ID_WIDTH-1:0] extension_completion_id;
    wire [SLOT_WIDTH-1:0] extension_completion_slot;

    wire [2:0] fp_retire_ready;
    wire [2:0] fp_retire_load_data_valid;
    wire [3*`RV64_XLEN-1:0] fp_retire_load_data;
    wire [2:0] fp_retire_result_valid;
    wire [2:0] fp_retire_private_write;
    wire [2:0] fp_retire_gpr_write;
    wire [3*`RV64_REG_ADDR_WIDTH-1:0] fp_retire_rd;
    wire [3*`RV64_XLEN-1:0] fp_retire_result_data;
    wire [2:0] fp_retire_fflags_valid;
    ...
    wire [`OPENRV64_INSTR_ID_WIDTH-1:0] fp_mem_complete_id =
        complete_id[2*`OPENRV64_INSTR_ID_WIDTH +:
                    `OPENRV64_INSTR_ID_WIDTH];
    wire [`OPENRV64_EXEC_COMPLETE_PAYLOAD_WIDTH-1:0]
        fp_mem_complete_payload = complete_payload[
            2*`OPENRV64_EXEC_COMPLETE_PAYLOAD_WIDTH +:
            `OPENRV64_EXEC_COMPLETE_PAYLOAD_WIDTH];
    wire fp_mem_complete_match = complete_valid[2] &&
        fp_entry_valid[fp_mem_complete_slot] &&
        (fp_entry_load[fp_mem_complete_slot] ||
         fp_entry_store[fp_mem_complete_slot]);
    wire fp_mem_complete_exception = fp_mem_complete_payload[
        `OPENRV64_COMPLETE_EXCEPTION_BIT] || fp_mem_complete_payload[
        `OPENRV64_COMPLETE_ILLEGAL_BIT];
    ...
    wire fp_mem_fault_valid = fp_mem_complete_match &&
        fp_mem_complete_exception;
    wire [`RV64_XLEN-1:0] fp_mem_complete_data =
        fp_mem_complete_payload[
            `OPENRV64_COMPLETE_DATA_LSB +: `RV64_XLEN];
    ...
    openrv64_dispatch_window_4pf #(
        ...
        .COUNT_WIDTH(DISPATCH_COUNT_WIDTH)
    ) u_dispatch (
        ...
        .decode_extension_valid_i(decode_extension_valid_i),
        .decode_extension_payload_i(decode_extension_payload_i),
        ...
        .allocation_extension_valid_o(allocation_extension_valid),
        .allocation_extension_payload_o(allocation_extension_payload),
        .allocation_branch_mask_o(allocation_branch_mask),
        .extension_entry_valid_i(fp_entry_valid),
        .extension_entry_compute_i(fp_entry_compute),
        .extension_entry_load_i(fp_entry_load),
        .extension_entry_store_i(fp_entry_store),
        .extension_operand_ready_i(fp_operand_ready),
        .extension_entry_store_data_i(fp_entry_store_data),
        ...
        .extension_mem_issue_valid_o(fp_mem_issue_valid),
        .extension_mem_issue_fire_o(fp_mem_issue_fire),
        .extension_mem_issue_is_load_o(fp_mem_issue_is_load),
        .extension_mem_issue_id_o(fp_mem_issue_id),
        .extension_mem_issue_slot_o(fp_mem_issue_slot),
        ...
        .queue_count_o(dispatch_occupancy_o)
    );

    wire [3*`RV64_REG_ADDR_WIDTH-1:0] rename_destination_arch;
    wire [2:0] rename_destination_valid;
    wire [3*PHYS_REG_ADDR_WIDTH-1:0] rename_destination_new_phys;
    wire [3*PHYS_REG_ADDR_WIDTH-1:0] rename_destination_old_phys;
    genvar fp_rename_lane;
    generate
        for (fp_rename_lane = 0; fp_rename_lane < 3;
             fp_rename_lane = fp_rename_lane + 1) begin : g_fp_rename_meta
            assign rename_destination_arch[
                fp_rename_lane*`RV64_REG_ADDR_WIDTH +:
                `RV64_REG_ADDR_WIDTH] = window_allocation_meta[
                    fp_rename_lane*`OPENRV64_DISPATCH_META_WIDTH + 35 +:
                    `RV64_REG_ADDR_WIDTH];
            assign rename_destination_valid[fp_rename_lane] =
                allocation_valid[fp_rename_lane] &&
                window_allocation_meta[
                    fp_rename_lane*`OPENRV64_DISPATCH_META_WIDTH + 17] &&
                (rename_destination_arch[
                    fp_rename_lane*`RV64_REG_ADDR_WIDTH +:
                    `RV64_REG_ADDR_WIDTH] != `RV64_REG_X0);
            assign allocation_meta[
                fp_rename_lane*RETIRE_META_WIDTH +:
                RETIRE_META_WIDTH] = {
                rename_destination_old_phys[
                    fp_rename_lane*PHYS_REG_ADDR_WIDTH +:
                    PHYS_REG_ADDR_WIDTH],
                rename_destination_new_phys[
                    fp_rename_lane*PHYS_REG_ADDR_WIDTH +:
                    PHYS_REG_ADDR_WIDTH],
                window_allocation_meta[
                    fp_rename_lane*`OPENRV64_DISPATCH_META_WIDTH +:
                    `OPENRV64_DISPATCH_META_WIDTH]
            };
        end
    endgenerate
    ...
    openrv64_rename_identity #(
        ...
        .source_arch_i(window_gpr_read_addr),
        .source_phys_o(gpr_read_addr),
    ...
    if (ENABLE_FD != 0) begin : g_fd_extension
        ...
        .squash_id_i(exec_redirect_id),
        .allocation_valid_i(allocation_extension_valid),
        .allocation_id_i(allocation_id),
        .allocation_slot_i(allocation_slot),
        .allocation_base_payload_i(decode_payload_i),
        .allocation_payload_i(allocation_extension_payload),
        ...
        .entry_fp_store_o(fp_entry_store),
        .entry_operand_ready_o(fp_operand_ready),
        .entry_mem_store_data_o(fp_entry_store_data),
        ...
        .fpu_src3_o(fpu_src3),
        .fpu_rd_o(fpu_rd),
        .fpu_fp_reg_write_o(fpu_fp_reg_write),
        ...
        .fpu_result_id_i(fpu_result_tag[
            0 +: `OPENRV64_INSTR_ID_WIDTH]),
        .fpu_result_slot_i(fpu_result_tag[
            `OPENRV64_INSTR_ID_WIDTH +: SLOT_WIDTH]),
        ...
        .retire_valid_i(queue_retire_valid),
        .retire_accept_i(queue_retire_accept),
        .retire_id_i(queue_retire_id),
        .retire_slot_i(queue_retire_slot),
        .retire_ready_o(fp_retire_ready),
        .retire_load_data_valid_o(fp_retire_load_data_valid),
        .retire_load_data_o(fp_retire_load_data),
        .retire_result_valid_o(fp_retire_result_valid),
        .retire_private_write_o(fp_retire_private_write),
        .retire_gpr_write_o(fp_retire_gpr_write),
        .retire_rd_o(fp_retire_rd),
        .retire_result_data_o(fp_retire_result_data),
    ...
    wire [2:0] fp_arch_private_write =
        fp_retire_private_write & retire_arch_o;
    reg fp_fpr_write;
    reg [`RV64_REG_ADDR_WIDTH-1:0] fp_fpr_write_addr;
    reg [`RV64_XLEN-1:0] fp_fpr_write_data;
    integer fp_commit_lane;
    always @* begin
        fp_fpr_write = 1'b0;
        fp_fpr_write_addr = {`RV64_REG_ADDR_WIDTH{1'b0}};
        fp_fpr_write_data = {`RV64_XLEN{1'b0}};
        for (fp_commit_lane = 0; fp_commit_lane < 3;
             fp_commit_lane = fp_commit_lane + 1) begin
            if (fp_arch_private_write[fp_commit_lane]) begin
                fp_fpr_write = 1'b1;
                fp_fpr_write_addr = fp_retire_rd[
                    fp_commit_lane*`RV64_REG_ADDR_WIDTH +:
                    `RV64_REG_ADDR_WIDTH];
                fp_fpr_write_data = fp_retire_result_data[
                    fp_commit_lane*`RV64_XLEN +: `RV64_XLEN];
            end
        end
    end
    ...
    assign fp_retire_fflags_valid_o = |fp_arch_fflags_valid;
    assign fp_retire_fflags_o =
        ({5{fp_arch_fflags_valid[0]}} & fp_retire_fflags[0 +: 5]) |
        ({5{fp_arch_fflags_valid[1]}} & fp_retire_fflags[5 +: 5]) |
        ({5{fp_arch_fflags_valid[2]}} & fp_retire_fflags[10 +: 5]);
    assign fp_retire_state_dirty_o =
        (|fp_arch_private_write) || fp_retire_fflags_valid_o;
...
`endif
    ...
    wire fp_state_lane0 = fp_retire_result_valid[0] &&
        (fp_retire_private_write[0] || fp_retire_fflags_valid[0]);
    wire fp_state_lane1 = fp_retire_result_valid[1] &&
        (fp_retire_private_write[1] || fp_retire_fflags_valid[1]);
    wire queue_csr_lane1 = queue_retire_commit[
        1*RETIRE_RESULT_WIDTH + `OPENRV64_RETIRE_RESULT_CSR_WRITE_BIT];
    wire queue_csr_lane2 = queue_retire_commit[
        2*RETIRE_RESULT_WIDTH + `OPENRV64_RETIRE_RESULT_CSR_WRITE_BIT];
    wire [2:0] retire_extension_ready = {
        fp_retire_ready[2] &&
            !(queue_csr_lane2 && (fp_state_lane0 || fp_state_lane1)),
        fp_retire_ready[1] && !(queue_csr_lane1 && fp_state_lane0),
        fp_retire_ready[0]
    };
    wire [3*`RV64_XLEN-1:0] fp_exception_tval = {
        {{(`RV64_XLEN-`RV64_INSTR_WIDTH){1'b0}},
         queue_retire_record[
             2*RETIRE_RECORD_WIDTH + `OPENRV64_RETIRE_ALLOC_INSTR_LSB +:
             `RV64_INSTR_WIDTH]},
        {{(`RV64_XLEN-`RV64_INSTR_WIDTH){1'b0}},
         queue_retire_record[
             1*RETIRE_RECORD_WIDTH + `OPENRV64_RETIRE_ALLOC_INSTR_LSB +:
             `RV64_INSTR_WIDTH]},
        {{(`RV64_XLEN-`RV64_INSTR_WIDTH){1'b0}},
         queue_retire_record[
             0*RETIRE_RECORD_WIDTH + `OPENRV64_RETIRE_ALLOC_INSTR_LSB +:
             `RV64_INSTR_WIDTH]}
    };
    openrv64_retire_3p #(
        .PHYS_REG_COUNT(PHYS_REG_COUNT),
        ...
        .extension_ready_i(retire_extension_ready),
        .extension_gpr_result_valid_i(fp_retire_gpr_write),
        .extension_gpr_result_i(fp_retire_result_data),
        .extension_exception_i(fp_retire_unsupported),
        .extension_cause_i({3{`RV64_EXCEPT_CAUSE_ILLEGAL_INSTR}}),
        .extension_tval_i(fp_exception_tval),
...
`ifndef SYNTHESIS
    initial begin
        if (ENABLE_ISSUE_WINDOW == 0)
            $fatal(1, "4PF requires the windowed dispatch path");
        if ((FREE_BRANCHES != 0) &&
            (ENABLE_ISSUE_WINDOW != 0))
