...
module openrv64_exec_bp_gshare_btb #(
    ...
    parameter integer HISTORY_BITS = PHT_INDEX_WIDTH,
    ...
    reg [HISTORY_BITS-1:0] speculative_ghr_q;
    reg [HISTORY_BITS-1:0] committed_ghr_q;
    ...
    reg [PHT_INDEX_WIDTH-1:0] inflight_pht_index_q
        [0:INFLIGHT_DEPTH-1];
    reg [HISTORY_BITS-1:0] inflight_ghr_before_q
        [0:INFLIGHT_DEPTH-1];
    ...
    wire [PHT_INDEX_WIDTH-1:0] lookup_pht_index =
        lookup_pc_index ^ speculative_ghr_q[PHT_INDEX_WIDTH-1:0];
    ...
    wire [HISTORY_BITS-1:0] resolved_history =
        {inflight_ghr_before_q[resolve_index][HISTORY_BITS-2:0],
         resolve_taken_i};
    ...
    wire [HISTORY_BITS-1:0] record_commit_history =
        (ENABLE_TAGGED_RESOLUTION != 0) ?
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            speculative_ghr_q <= {HISTORY_BITS{1'b0}};
            committed_ghr_q <= {HISTORY_BITS{1'b0}};
            ...
            if (flush_i) begin
                ...
                if (lookup_allocate_i && !queue_full) begin
                    ...
                    if (lookup_branch_i)
                        speculative_ghr_q <=
                            {speculative_ghr_q[HISTORY_BITS-2:0],
    ...
    // Table contents are not reset.  Their resettable valid vectors provide
    // cold behavior, while keeping these arrays eligible for SRAM inference.
    always @(posedge clk) begin
        if (rst_n && resolve_has_record &&
            inflight_branch_q[resolve_index]) begin
            if (!pht_valid_q[resolved_pht_index])
            ...
            end else if (pht_counter_q[resolved_pht_index] != 0)
                pht_counter_q[resolved_pht_index] <=
                    pht_counter_q[resolved_pht_index] - 1'b1;
        end
...
`ifndef SYNTHESIS
    initial begin
        ...
        if ((HISTORY_BITS < 2) || (HISTORY_BITS < PHT_INDEX_WIDTH))
            $fatal(1,
                   "gshare HISTORY_BITS must be >= PHT index width and >= 2");
        if (COUNTER_BITS < 2)
